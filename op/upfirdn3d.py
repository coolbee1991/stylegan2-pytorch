...
def upfirdn2d(input, kernel, up=1, down=1, pad=(0, 0)):
    ...
    if len(pad) == 2:
        pad = (pad[0], pad[1], pad[0], pad[1], pad[0], pad[1])
...
def upfirdn3d_native(
    ...
    out = F.pad(out, [0, 0, 0, up_z - 1, 0, 0, 0, up_x - 1, 0, 0, 0, up_y - 1])
    out = out.view(-1, in_h * up_y, in_w * up_x, in_d * up*z, minor)

    out = F.pad(
        out, [0, 0, max(pad_z0, 0), max(pad_z1, 0), max(pad_x0, 0), max(pad_x1, 0), max(pad_y0, 0), max(pad_y1, 0)]
    )
    out = out[
        :,
